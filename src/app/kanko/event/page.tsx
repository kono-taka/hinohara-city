...
const events = [
  ...
  { month: 6, day: 14, weekday: "土", title: "第19回 日野原やまびこ祭り（1日目）", place: "中央公園特設会場", time: "12:00〜21:00", fee: "入場無料", desc: "市内最大の夏祭り。地元グルメ屋台・ステージ・盆踊り・花火大会（1日目）。" },
  { month: 6, day: 15, weekday: "日", title: "第19回 日野原やまびこ祭り（2日目）", place: "中央公園特設会場", time: "10:00〜20:00", fee: "入場無料", desc: "2日目は子ども向けイベントを多数開催。大抽選会、フィナーレパレードあり。" },
          ...
          <div style={{ border: "1px solid var(--border-light)", background: "#fff", marginBottom: 16 }}>
            {events.map((ev, i) => (
              <div key={i} style={{ display: "flex", gap: 14, padding: "12px 14px", borderBottom: "1px solid #e8edf4", alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: "bold", fontSize: 14, marginBottom: 4 }}>{ev.title}</div>
